...
import { TaskNodeData, StatusConfig } from '../types';
...
const TaskNode: React.FC<NodeProps<TaskNodeData>> = ({ data, selected }) => {
  ...
  return (
    <div className={`
      ...
      ${config.borderColor} ${config.bgColor} ${config.hoverBorder} ${config.hoverBg}
      ${selected 
    ...
    `}>
      ...
      <div className="flex items-center gap-3 relative z-10">
        ...
        <div className="flex items-center justify-center">
          <StatusIcon className={`w-4 h-4 ${config.iconColor} transition-colors duration-200`} />
        </div>
        ...
        <div className={`w-2 h-2 ${config.dotColor} rounded-full shadow-sm flex-shrink-0`} />
      </div>
