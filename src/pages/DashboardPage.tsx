...
import { Plus, FolderOpen, Calendar, Users, Sparkles, ArrowRight, Clock, CheckCircle, AlertCircle, Loader2, Trash2, MoreVertical, Home, Share2, Settings, Menu } from 'lucide-react';
...
const DashboardPage: React.FC = () => {
  ...
  const handleCreateProject = async (projectData: ProjectFormData): Promise<void> => {
    try {
      const newProject = await createProject({
        name: projectData.name,
        description: projectData.description || 'A new project ready for planning and organization',
      ...
      const url = `/project/${newProject.id}${searchParams.toString() ? `?${searchParams.toString()}` : ''}`;
      navigate(url);
    } catch (err) {
  ...
  const handleOpenDeleteModal = (project: Project, event: React.MouseEvent): void => {
  ...
  const handleToggleDropdown = (projectId: string, event: React.MouseEvent): void => {
    event.stopPropagation(); // Prevent opening the project
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-background-900 via-background-800 to-background-900 flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-secondary-400 font-medium">Loading your projects...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
          ...
          <div className="mb-6 sm:mb-8">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 sm:mb-6 gap-4">
              <div>
                <h2 className="text-xl sm:text-2xl font-bold text-white tracking-tight mb-2">Welcome back</h2>
              ...
              {projects.length > 0 && (
                <div className="text-left sm:text-right">
                  <div className="text-sm text-secondary-400 font-medium">Total Progress</div>
              ...
              <div className="bg-background-800/40 backdrop-blur-xl border border-background-700/30 rounded-xl p-4 sm:p-6 shadow-lg relative overflow-hidden">
                ...
                <div className="relative flex items-center gap-3 sm:gap-4">
                  ...
                  <div>
                    ...
                    <div className="text-xs sm:text-sm text-secondary-400 font-medium">Total Projects</div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-background-800/40 backdrop-blur-xl border border-background-700/30 rounded-xl p-4 sm:p-6 shadow-lg relative overflow-hidden">
                ...
                <div className="relative flex items-center gap-3 sm:gap-4">
                  ...
                  <div>
                    ...
                    <div className="text-xs sm:text-sm text-secondary-400 font-medium">Active Projects</div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-background-800/40 backdrop-blur-xl border border-background-700/30 rounded-xl p-4 sm:p-6 shadow-lg relative overflow-hidden">
                ...
                <div className="relative flex items-center gap-3 sm:gap-4">
                  ...
                  <div>
                    ...
                    <div className="text-xs sm:text-sm text-secondary-400 font-medium">Completed</div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-background-800/40 backdrop-blur-xl border border-background-700/30 rounded-xl p-4 sm:p-6 shadow-lg relative overflow-hidden">
                ...
                <div className="relative flex items-center gap-3 sm:gap-4">
                  ...
                  <div>
                    ...
                    <div className="text-xs sm:text-sm text-secondary-400 font-medium">Total Tasks</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="space-y-4 sm:space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
              <h3 className="text-lg sm:text-xl font-bold text-white tracking-tight">Your Projects</h3>
              {projects.length > 0 && (
                ...
                return (
                  ...
                  >
                    ...
                    <div className="relative flex items-start justify-between mb-4">
                      <div className={`p-2 sm:p-3 bg-gradient-to-r ${project.color} rounded-lg shadow-lg`}>
                        <FolderOpen className="w-5 h-5 sm:w-6 sm:h-6 text-white" />
                      </div>
                      <div className="flex items-center gap-2">
                        <div className="flex items-center gap-2 text-sm">
                          {getStatusIcon(project.status)}
                          <span className="text-secondary-400 font-medium hidden sm:inline">{project.status}</span>
                        </div>
                    ...
                    <div className="relative grid grid-cols-2 gap-4 mb-6">
                      <div className="text-center p-3 bg-background-700/20 rounded-lg">
                        <div className="text-base sm:text-lg font-bold text-white">{project.phases}</div>
                        <div className="text-xs text-secondary-400 font-medium">Phases</div>
                      </div>
                      <div className="text-center p-3 bg-background-700/20 rounded-lg">
                        <div className="text-base sm:text-lg font-bold text-white">
                          {project.completedTasks}/{project.tasks}
                        </div>
                        <div className="text-xs text-secondary-400 font-medium">Tasks</div>
                      </div>
                    </div>
          ...
          {projects.length === 0 && (
            <div className="text-center py-12 sm:py-16">
              ...
              <h3 className="text-xl sm:text-2xl font-bold text-white mb-4">No Projects Yet</h3>
              <p className="text-secondary-400 mb-6 sm:mb-8 max-w-md mx-auto px-4">
                Get started by creating your first project or use the sample data above to explore the features.
              </p>
              <button
                onClick={() => setIsModalOpen(true)}
